...
function waitForGoogleSDK() {
    if (typeof google !== 'undefined' && google.accounts) {
        ...
        setTimeout(waitForGoogleSDK, 500);
    }
}
...
function initGoogleClient() {
    AppState.tokenClient = google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
            ...
            localStorage.setItem(STORAGE_KEYS.TOKEN, AppState.accessToken);
            localStorage.setItem(STORAGE_KEYS.EXPIRE, newExpireTime);
...
async function uploadImageToDrive(blob, filename, imgIndex) {
    try {
        ...
        if (!AppState.folderId) throw new Error('無法初始化雲端資料夾');
...
function resetPhotoArea(index, placeholderText = null) {
    ...
    $('ph' + index).innerText = placeholderText || `輕觸上傳相片 (區${index})`;
    $('file' + index).value = '';
